...
2018/02/26(ZhangXiaomou)
1.增加时序合闸信号检测，注意开入量扫描增加时应该在合分闸信号检测之前增加，判断在CheckAllLoopSwitchState函数内进行
2.将程序转移到内部GIT中进行版本控制

2017/10/18(ZhangXiaomou)
1.修改部分注释格式
2.增加在获取参数后立即进行一次温度数据的获取，测试使用

2017/09/21(ZhangXiaomou)
1.程序放置到台式机中，现在已台式机为主
2.修改注释

2017/8/24 (ZhangXiaomou)
1.添加远方就地、带电、工作模式信息上传;

2017/8/15 (ZhangXiaomou)
1.修改读取定值范围至0xFF后产生的BUG;

2017/8/06 (ZhangXiaomou)
1.增加一个温度传感器初始化检测

2017/8/04 (ZhangXiaomou)
1.发现拒动现象
2.更新拒动检测时间计算形式

2017/8/01(ZhangXiaomou)
1.DeviceNet函数初始化时将IdentifierObj.device_state标志位清零初始化

2017/7/20
1.改A在预制之后打开中断3，接收脉冲序列，为长时间接收脉冲序列，并在启动时根据控制位选择。
消除BC预制单独预制执行时的问题。
...
2017/07/15(ZhangXiaomou)
1.测试中发现BUG，在按键按下之后上锁，但是命令不符合未进行解锁操作

2017/07/14(修改人：ZhangXiaomou)
1.增加CAN总线被动错误检测，超时则关闭CAN中断，并复位CAN
2.增加信号检测功能
3.修复同步合闸BUG
4.增加分合闸时间补偿常量，为CompensationTime = 4ms;
5.更改拒动检测位置
6.在有命令到达时立即返回
7.增加错误代码
//*****************************************************************
...
2017/07/13(修改人：ZhangXiaomou)
1.修改电容电压状态检测形式
2.修复部分BUG
3.修复部分存在潜在危险的程序
4.更改按键扫描形式
5.使用const定义常量数组
6.增加DeviceNet超时离线检测(超时则复位),简化DeviceNet建立连接

2017/07/12(修改人：ZhangXiaomou)
1.解决按键状态与电容状态突变后的上传问题
2.更改变量名称
3.预制死机问题重现

2017/7/3
1.修正时间为清0的情况
2.重新修整ActionAttribute参数
3.重构合闸设置，但没有拒动检测。
4.重构分闸设置，但没有拒动检测。
5.合并合分闸设置。
6.修改大选相按钮控制顺序，添加宏定义位
...
2017/05/16(修改人：ZhangXiaomou)
修改在做EMC实验时，发生的程序上的BUG。

2017/05/17(修改人：ZhangXiaomou)
完成心跳数据上传函数的编写
完善按键扫描函数
升级分合闸次数写EEPROM的函数

2017/05/21(修改人：ZhangXiaomou)
完成看门狗喂狗函数的添加
添加了同步信号识别函数
绘制完成偏移时间判断的函数，未完成程序的设计

2017/05/22(修改人：ZhangXiaomou)
更改同步信号触发方式为PWM形式，且不采用中断方式，而采用循环查找的方式，频率在10K~11K之间
更改同步合闸控制函数，使用索引值对其顺序进行控制
删除定时器计时的-2操作

2017/05/23(修改人：ZhangXiaomou)
完成状态上传函数的修改
完成同步合闸先后顺序和偏移时间的整定
18:30 整理程序中的宏定义，删除多余的宏定义，更改一些无意义的定义


2017/05/28(修改人：ZhangXiaomou)
更改计时方式
更改同步信号识别方式
优化同步合闸逻辑控制
优化两相同步控制逻辑
...
2017/06/02(修改人：ZhangXiaomou)
解决总合闸按钮失效的错误
将整个的按钮控制修改为使用本合同的按钮控制逻辑（即按钮控制不考虑本地或者远方，使用硬件实现）
待解决问题：CAN总线发送错误问题

2017/06/03(修改人：ZhangXiaomou)
解决单相控制合分闸未执行的错误
添加拒动处理函数
...
2017/06/05(修改人：ZhangXiaomou)
增加按键判断条件
编写IIC模拟函数以及驱动SD2405时钟芯片
增加电容跌落电压测量
完成SD2405的驱动程序

2017/06/06(修改人：ZhangXiaomou)
解决温度传感器上传值为0的问题
解决IIC总线读取时钟芯片出错问题
在时钟芯片内部写入标志位0xAA
修改EEPROM注释
...
2017/06/27(修改人：ZhangXiaomou)
精简DeviceNet中的部分函数

2017/06/28(修改人：ZhangXiaomou)
将所有依靠看门狗超时检测的都修改为软件检测
将CAN中断中的定值写入、监控数据读取，均转移到主程序中
修改同步预制函数中的BUG，修改同步控制中回路起始为0b01
修复部分BUG

2017/06/29(修改人：ZhangXiaomou)
修改同步预制应答
修改同步信号识别函数
精简程序，编译级别调高

2017/07/04(修改人：张宇飞)
2017/07/10(修改人：ZhangXiaomou)
1.优化继电器和LED灯状态输出，使其扫描时间缩短了一半
2.将电容的电压读取和显示分开执行
3.优化了按键扫描策略
4.优化165扫描读取函数

2017/07/11(修改人：ZhangXiaomou)
删除了Rtu文件
更改了电容状态更新函数
优化了分合位检测更新函数
添加了继电器、LED灯的数组操作
