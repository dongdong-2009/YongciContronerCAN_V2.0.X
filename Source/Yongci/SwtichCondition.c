...
uint8 CheckIOState(void)
{
    ...
    switch (g_Order)
    {
        ...
        case CHECK_3_HE_ORDER: //收到机构2合闸命令
        {
            ClrWdt();        
            if(CAP3_STATE)  //判断第三块驱动是否存在
            {
...
void SwitchScan(void)
{
    ...
    //远方\就地检测
    if(YUAN_BEN_CONDITION() && (g_lockflag[8] == 0))
    {
        ClrWdt();
        g_timeCount[17]++;
    ...
    else if(!YUAN_BEN_CONDITION() && (g_lockflag[9] == 0))
    {
        ClrWdt();
        g_timeCount[18]++;
    ...
    //工作\调试模式检测
    if(WORK_DEBUG_CONDITION() && (g_lockflag[10] == 0))
    {
        ClrWdt();
        g_timeCount[19]++;
    ...
    else if(!WORK_DEBUG_CONDITION() && (g_lockflag[11] == 0))
    {
        ClrWdt();
        g_timeCount[20]++;
